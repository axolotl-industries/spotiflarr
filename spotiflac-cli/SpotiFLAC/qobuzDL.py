...
class QobuzDownloader:
    ...
    def _download_from_spotbye(self, base_url: str, param_name: str, track_id: int, value: str) -> str:
        """Spotbye's Qobuz proxies (extracted from SpotiFLAC-Next AppImage).
        Path-based URL: e.g. https://qbzalt.spotbye.qzz.io/{track_id}?quality=6
        Response shape unverified — try the same parsing as the standard APIs.
        """
        url = f"{base_url}/{track_id}?{param_name}={value}"
        headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"}
        resp = self.session.get(url, headers=headers, timeout=self.timeout)
        ...
        try:
            ...
            # Possibility: server XOR-encodes like jumo-dl. Try that.
            try:
                decoded = decode_xor(resp.content)
                data = json.loads(decoded)
            except:
                raise Exception("Invalid JSON response (and XOR decode failed)")
        ...
        raise Exception("Invalid Spotbye response structure")
    ...
    def get_download_url(self, track_id: int, quality: str, allow_fallback: bool) -> str:
        ...
        def attempt_download(qual):
            # Spotbye proxies — extracted from SpotiFLAC-Next AppImage
            # (May 2026). The standard APIs below are mostly dead so Spotbye
            # gets first crack. Path-based URLs: /<id>?quality=N or ?format_id=N.
            format_id = str({"6": 6, "7": 7, "27": 27}.get(qual, 6))
            providers = [
                {"name": "Spotbye(qbzalt)",
                 "func": lambda: self._download_from_spotbye(
                     "https://qbzalt.spotbye.qzz.io", "quality", track_id, qual)},
                {"name": "Spotbye(jmdl)",
                 "func": lambda: self._download_from_spotbye(
                     "https://jmdl.spotbye.qzz.io", "format_id", track_id, format_id)},
            ]

            fallback_providers = []
            for api in standard_apis:
                fallback_providers.append({"name": f"Standard({api})",
                                           "func": lambda a=api: self._download_from_standard(a, track_id, qual)})
            fallback_providers.append({"name": "Jumo-DL",
                                       "func": lambda: self._download_from_jumo(track_id, qual)})
